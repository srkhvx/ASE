    ...
    for x, crime, y in zip(data["Zip Code"], data["Offense"], data["Reported_Date"]):
        try:
            ...
            if (dist < threshold ):
                ...
                db1.reviews.insert_one({'Longiture': select_zipcode(x)['long'],'Latitude': select_zipcode(x)['lat'],'Offense': crime,'Date': y})
...
if __name__ == "__main__":
    ...
    result=temporary(5,'x',5,64112)

    print(result)
    collection = db1.reviews  # selecting the coll1 in myDatabase
